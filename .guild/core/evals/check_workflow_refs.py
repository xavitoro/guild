...
and run records that plain JSON Schema validation cannot express:
  ...
  - Every workflow step's responsible_profile is either 'human' or a real
    agent profile id under .guild/core/agents/.
...
def load_all(base: Path, glob_pattern: str) -> dict[str, dict]:
    """Loads YAML files matching glob_pattern under base, keyed by their declared id."""
    out = {}
...
def main() -> int:
    ...
    runs = load_all(STATE_ROOT, "runs/*.yaml")
