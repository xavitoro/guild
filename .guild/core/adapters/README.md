...
Provider-specific files generated from the canonical `.guild/core/agents/`
and `.guild/core/skills/` definitions. `.guild/core/` remains the source of
truth — nothing under `.agents/`, `.claude/`, or the generated sections of
root `AGENTS.md`/`CLAUDE.md` is hand-maintained.

## The generator

[`generate_adapters.py`](generate_adapters.py) is the deterministic
sync/generation command. It reads every `.guild/core/agents/*/manifest.yaml`
and `.guild/core/skills/*/SKILL.yaml`, and produces:
...
skills nor subagents read `.guild/core/agents/`, `.guild/core/skills/` and
`.guild/core/workflows/` directly — see
...
Run this as part of `.guild/core/evals/` validation — see
...
`.guild/core/policies/default-policies.yaml` (`merge_protected_branch`, in
