...
- WI-001 is complete: 16 JSON Schemas live under `.guild/schemas/`, artifact
  templates under `.guild/templates/`, and `.guild/evals/validate_guild.py`
...
- WI-002 is complete: all fourteen agent profiles exist under
  `.guild/agents/<id>/` (manifest + `AGENT.md`), independent QA (Barbarian)
  and security (Rogue) gates are preserved, and no profile has unrestricted
  write/merge/deploy/secret access. `.guild/evals/check_agent_profiles.py`
  proves the roster is complete, schema-valid and unique.
- WI-003 is complete, closing milestone M1: 22 reusable skills under
  `.guild/skills/`, all six canonical workflows under `.guild/workflows/`
  ...
  records under `.guild/runs/`. `.guild/evals/check_workflow_refs.py` proves
  every profile/skill/workflow/run cross-reference resolves.
- WI-004 is complete, closing milestone M2: `.guild/adapters/generate_adapters.py`
  ...
  drift detection. `.guild/evals/smoke_test_add_feature.py` proves full
  adapter coverage for the `add-feature` workflow.
- WI-005 is complete, closing milestone M3 and the full M1–M3 bootstrap
  roadmap. No dedicated prompt existed for it; scope was derived from
  `GUILD_MASTER_SPEC.md` section 14's definition of done. Added
  `check_independent_gates.py` (structural QA/security independence + Red
  gates owned by `human`), `check_language_neutrality.py` (deny-list scan),
  and `pilot_install_check.py` (installs an isolated copy of `.guild/` into
