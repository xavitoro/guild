...
Checks, over every YAML/JSON file under .guild/:
  1. Syntax   - the file parses as valid YAML/JSON.
  2. Schemas  - every *.schema.json file under .guild/schemas/ is itself a
...
SCHEMAS_DIR = GUILD_ROOT / "schemas"
TEMPLATES_DIR = GUILD_ROOT / "templates"
    ...
    for path, data in parsed_files.items():
        ...
        if schema is None:
            errors.append(
                ...
                f"has no matching file under .guild/schemas/"
            )
            continue
