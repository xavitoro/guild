...
For every step in .guild/workflows/add-feature/workflow.yaml, checks that:
...
replacement for .guild/evals/check_workflow_refs.py (which checks every
workflow's references against the canonical catalogue, independent of
whether adapters have been generated).
...
WORKFLOW_PATH = GUILD_ROOT / "workflows" / "add-feature" / "workflow.yaml"
...
def main() -> int:
    ...
    for step in workflow["steps"]:
        ...
        if profile != "human" and profile not in profiles_seen:
            ...
            check_generated_file(
                agent_path, f".guild/agents/{profile}/manifest.yaml", errors
            )
        ...
        if skill not in skills_seen:
            ...
            check_generated_file(codex_skill_path, f".guild/skills/{skill}/SKILL.yaml", errors)
            check_generated_file(claude_skill_path, f".guild/skills/{skill}/SKILL.yaml", errors)
    ...
    drift = subprocess.run(
        [sys.executable, str(GUILD_ROOT / "adapters" / "generate_adapters.py"),
