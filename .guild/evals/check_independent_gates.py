...
Checks:
  1. Every one of the 14 agent manifests forbids every Red-tier action
     listed in .guild/policies/default-policies.yaml's approval_required
...
def main() -> int:
    ...
    policies = yaml.safe_load((GUILD_ROOT / "policies" / "default-policies.yaml").read_text(encoding="utf-8"))
    ...
    if QA_ID in agents:
        ...
        errors.append(f"[missing-profile] {QA_ID} not found under .guild/agents/")
    ...
    if SECURITY_ID in agents:
        ...
        errors.append(f"[missing-profile] {SECURITY_ID} not found under .guild/agents/")
