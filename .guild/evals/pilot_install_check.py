...
Follows .guild/adapters/INSTALL.md's "into an empty repository" steps
literally, against an isolated temporary directory that has nothing else
from this bootstrap pack (no AGENTS.md, CLAUDE.md, bootstrap-prompts/):

  1. Copy .guild/ into a fresh temp directory.
  ...
  4. Re-run every other .guild/evals/check_*.py and validate_guild.py
     *from the copy* (proving .guild/ is self-contained and doesn't
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="guild-pilot-") as tmp:
        ...
        target.mkdir()
        shutil.copytree(GUILD_ROOT, target / ".guild")
        print(f"Pilot target: {target} (isolated; contains only a copy of .guild/)")

        gen = run([sys.executable, str(target / ".guild" / "adapters" / "generate_adapters.py"),
        ...
        for script in OTHER_CHECKS:
            result = run([sys.executable, str(target / ".guild" / "evals" / script)], cwd=target)
            status = "OK" if result.returncode == 0 else "FAILED"
            print(f"  [{status}] {script} (from the pilot copy)")
            if result.returncode != 0:
                errors.append(f"[{script}-failed] {result.stdout.strip()}\n{result.stderr.strip()}")

        drift = run([sys.executable, str(target / ".guild" / "adapters" / "generate_adapters.py"),
    ...
    print("OK: a fresh, isolated copy of .guild/ installs and validates cleanly end to end.")
    return 0
